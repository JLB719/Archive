...
public final class PLTreeNode implements PLTreeNodeInterface
{
	...
	public String toStringPrefix()
	{
		...
		if (type.getArity() == 2) {
			return type.getPrefixName() + "(" + child1.toStringPrefix() + " , " + child2.toStringPrefix() + ")";
		}
		return null;
	}
	...
	public void applyVarBindings(Map<NodeType, Boolean> bindings)
	{
		...
		if (type.getArity() == 0) {
			if(bindings.get(type) != null && bindings.get(type) == true) {
				this.type = NodeType.TRUE;
			} else if (bindings.get(type) != null && bindings.get(type) == false) {
	...
	{
		// WRITE YOUR CODE HERE, CHANGING THE RETURN STATEMENT IF NECESSARY
		// need to look at arity when recursing have if statements
		// if recursing down 
		return null;
	}
	...
	{
		// WRITE YOUR CODE HERE
		// use while loop with counter then modulo towo 
