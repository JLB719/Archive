...
public class MLNodeTest
{
	...
	{
		MLNodeInterface<String> n = new MLNode<>("abc"); // creates a node 
		assertEquals("Initialization of MLNode failed on item", "abc", n.getElement()); // abc is what it expects n.getElement() is what is there so need to be able to use the get method to get an element correctly
		assertSame("Initialization of MLNode failed on prev1", n, n.getPrev1()); //expects n gets n.previous so need to make sure some kind of list
		assertSame("Initialization of MLNode failed on next1", n, n.getNext1()); // same thing for the rest
	...
	public void testAddElement()
	{
		...
		assertSame("adding1", n3, n0.getPrev1()); // so n3 shoud have the previous value of n0 on list 1
		assertSame("adding1", n0, n1.getPrev1()); // same thing for the rest
